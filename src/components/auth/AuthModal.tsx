...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const currentOrigin =
    typeof window !== 'undefined' && window.location.origin && !window.location.origin.includes('file://')
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/unauthorized-domain' || err.message?.includes('unauthorized-domain')) {
        ...
        setErrorMsg(
          'Phương thức đăng nhập này chưa được bật trên Firebase Console (Authentication -> Sign-in method). Vui lòng kích hoạt "Email/Password" hoặc "Google".'
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        setErrorMsg(
          'Phương thức Đăng nhập bằng Email/Mật khẩu chưa được bật trong dự án Firebase. Vui lòng bật "Email/Password" trong Firebase Console -> Authentication -> Sign-in method.'
        );
      } else {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/75 dark:bg-black/80 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 animate-fade-in">
      <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border-t sm:border border-slate-200/80 dark:border-slate-800/80 rounded-t-[28px] sm:rounded-3xl shadow-2xl shadow-black/30 w-full max-w-md overflow-hidden flex flex-col max-h-[92dvh] sm:max-h-[90vh]">
        ...
        {isLoggedIn && !isSwitchingAccount ? (
          <>
            {/* Header */}
            <div className="p-4 sm:p-6 pb-3 sm:pb-4 border-b border-slate-100/80 dark:border-slate-800 flex items-center justify-between bg-slate-50/90 dark:bg-slate-900 backdrop-blur-sm shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 text-[9px] sm:text-[10px] font-black uppercase bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 rounded-md border border-emerald-200 dark:border-emerald-800/80 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                      Đã Đăng Nhập
                    </span>
                  </div>
                  <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
                    Đồng bộ tự động dữ liệu mẫu tem in qua Cloud Firestore
                  </p>
                </div>
              </div>
              {!required && (
            ...
            <div className="p-4 sm:p-6 space-y-4 max-h-[85vh] overflow-y-auto">
              ...
              <div className="p-4 bg-gradient-to-br from-indigo-50/90 via-white to-blue-50/90 dark:from-indigo-950/60 dark:via-slate-900 dark:to-indigo-950/80 rounded-2xl shadow-xs dark:shadow-md border border-indigo-200/80 dark:border-indigo-800/60 flex items-center gap-3.5">
                <div className="w-13 h-13 rounded-2xl bg-gradient-to-tr from-indigo-600 to-blue-600 text-white flex items-center justify-center text-lg font-black uppercase shadow-md shadow-indigo-500/25 shrink-0 ring-3 ring-indigo-200 dark:ring-indigo-800">
                  {authUser?.photoURL ? (
                    <img
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300 font-medium truncate mt-0.5">
              ...
              <div className="p-3.5 bg-emerald-50/90 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 rounded-2xl text-xs space-y-2">
                ...
                <p className="text-emerald-900/80 dark:text-emerald-200/90 text-[11px] leading-relaxed">
                  Mọi mẫu tem, thông số nhãn, danh sách in Excel và cài đặt máy in của bạn đang được tự động lưu trữ và đồng bộ hóa an toàn.
                </p>
                <div className="pt-1.5 grid grid-cols-2 gap-2 text-[10px] text-emerald-900 dark:text-emerald-200 font-medium border-t border-emerald-200/60 dark:border-emerald-800/40">
              ...
              <div className="space-y-2.5 pt-2">
                {onLogout && (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-red-50 hover:bg-red-100 dark:bg-red-950/60 dark:hover:bg-red-900/60 text-red-600 dark:text-red-300 font-bold text-xs rounded-xl border border-red-200 dark:border-red-800/60 transition-colors cursor-pointer shadow-xs active:scale-98"
                  >
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-xs rounded-xl transition-colors cursor-pointer active:scale-98"
                >
                ...
                {!required && (
                  <button
                    type="button"
                    onClick={onClose}
                    className="w-full py-2.5 px-4 bg-slate-900 hover:bg-slate-800 text-white dark:bg-indigo-600 dark:hover:bg-indigo-700 font-bold text-xs rounded-xl transition-colors cursor-pointer shadow-sm active:scale-98"
                  >
                    Xong / Đóng Cửa Sổ
                  </button>
                )}
          ...
          <>
              ...
              <div className="flex items-center gap-3">
                {isSwitchingAccount ? (
                  <button
                    ...
                    className="p-2 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors cursor-pointer"
                    title="Quay lại tài khoản hiện tại"
                  >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {required && (
                      <span className="px-2 py-0.5 text-[9px] sm:text-[10px] font-black uppercase bg-red-100 dark:bg-red-950/80 text-red-700 dark:text-red-300 rounded-md border border-red-200 dark:border-red-800/80">
                        Bắt buộc
                      </span>
                    )}
                  </div>
                  <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
                    {required
                      ...
                      : 'Lưu và đồng bộ hóa cài đặt mẫu in qua Cloud'}
                  </p>
                </div>
              </div>
              {!required && (
            ...
            <div className="p-4 sm:p-6 space-y-4 max-h-[85vh] overflow-y-auto">
              {/* App Branding Card */}
              <div className="p-4 bg-gradient-to-br from-indigo-50/90 via-white to-blue-50/90 dark:from-indigo-950/60 dark:via-slate-900 dark:to-indigo-950/80 rounded-2xl shadow-xs dark:shadow-md border border-indigo-200/80 dark:border-indigo-800/60 space-y-2">
                <div className="flex items-center gap-2.5">
                  <div className="p-2 rounded-xl bg-indigo-600 text-white shadow-xs">
                    <QrCode className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">QR Label Pro</h4>
                    <p className="text-[10px] text-slate-500 dark:text-indigo-200/80">
                      Thiết kế & In tem nhãn mã vạch nhiệt chuyên nghiệp
                    </p>
                  </div>
                </div>

                <div className="pt-1.5 grid grid-cols-2 gap-1.5 text-[10px] text-slate-700 dark:text-indigo-100/90 font-medium border-t border-indigo-200/60 dark:border-indigo-800/40">
                  <div className="flex items-center gap-1.5">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
              ...
              {errorMsg === 'auth/unauthorized-domain' ? (
                <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/80 rounded-2xl text-xs space-y-2.5 animate-fade-in">
                  ...
                  <p className="text-amber-900/80 dark:text-amber-200/90 leading-relaxed text-[11px]">
                    Tên miền{' '}
                    <code className="font-mono bg-amber-100 dark:bg-amber-900/60 px-1.5 py-0.5 rounded text-[10px] font-bold select-all text-amber-900 dark:text-amber-100">
                      {typeof window !== 'undefined' ? window.location.hostname : 'domain-cua-ban'}
                    </code>{' '}
                    chưa được ủy quyền trong Firebase Console.
                  </p>
                  <div className="pt-1 space-y-1 text-[11px] text-slate-700 dark:text-slate-300">
                    ...
                    <ol className="list-decimal pl-4 space-y-1">
                      <li>
                        Vào <strong>Firebase Console</strong> &gt; project của bạn &gt; <strong>Authentication</strong>{' '}
                        &gt; <strong>Settings</strong> &gt; <strong>Authorized domains</strong>.
                      </li>
                      <li>
                        Bấm <strong>Add domain</strong> và dán tên miền:{' '}
                        <code className="font-mono bg-amber-100 dark:bg-amber-900/60 px-1.5 py-0.5 rounded text-[10px] font-bold select-all text-amber-900 dark:text-amber-100">
                    ...
                    <div className="mt-2 text-slate-500 font-medium">
                      Hoặc bạn có thể Đăng Nhập bằng Email/Mật khẩu bên dưới ngay lập tức!
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/80 rounded-2xl text-xs space-y-2.5 animate-fade-in">
                  ...
                  <p className="text-amber-900/80 dark:text-amber-200/90 leading-relaxed text-[11px]">
                    Google Firebase quy định bảo mật chặn cửa sổ Popup 1-click khi chạy từ file máy tính cục bộ (
                    <code className="font-mono bg-amber-100 dark:bg-amber-900/60 px-1 py-0.5 rounded text-[10px]">
                      file://
                    </code>
                    ).
                  </p>
                  ...
                  <div className="pt-1 space-y-1 text-[11px]">
                    <div className="font-semibold text-amber-900 dark:text-amber-100">
                      👉 Đăng nhập nhanh trực tiếp trên app EXE:
                    </div>
                    <ul className="list-disc pl-4 space-y-0.5 text-slate-700 dark:text-slate-300">
                      <li>
                        Vui lòng gõ <strong>Email + Mật khẩu</strong> ở khung bên dưới.
                      </li>
                      <li>
                        Nếu chưa có tài khoản, gõ Email của bạn và bấm <strong>&ldquo;Tạo Tài Khoản Mới&rdquo;</strong>.
                      </li>
                    </ul>
                  </div>
                </div>
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 py-2.5 px-4 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs rounded-2xl shadow-xs transition-all cursor-pointer disabled:opacity-50"
              >
              ...
              <form onSubmit={handleSubmit} className="space-y-3.5">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] text-slate-400 leading-tight">Bấm hình hoặc nút để đổi mã</span>
                  </div>
